...
class CDCLSolver {
	private:
	// a vector of state of literals with assignment as follows:
	// 1 : true
	// 0 : false
	// -1 : unassigned
	vector<int> literals;
...
void CDCLSolver::initialize(){
	...
	cin >> literal_count;
	cin >> clause_count;
	assigned_literal_count = 0; // No literal is assigned till now
	kappa_antecedant = -1;
	pick_counter = 0;
...
int CDCLSolver::CDCL() {
	...
	int unit_propagation_result = unit_propagate(decision_level);
	if (unit_propagation_result == RetVal::r_unsatisfied){
		return unit_propagation_result;
	}
	while (!all_variables_assigned()){
		...
		while (true){
			unit_propagation_result = unit_propagate(decision_level);
			if (unit_propagation_result == RetVal::r_unsatisfied){
				if (decision_level == 0){
					return unit_propagation_result;
				}
...
int CDCLSolver::unit_propagate(int decision_level){
	...
	do {
		...
		for (int i = 0; i < literal_list_per_clause.size(); i++){
			...
			// Iterate over all literals in the ith clause
...
void CDCLSolver::assign_literal(int variable, int decision_level, int antecedent){
...
void CDCLSolver::unassign_literal(int literal_index){
	literals[literal_index] = -1;
...
int CDCLSolver::literal_to_variable_index(int variable){
	return (variable > 0) ? variable - 1 : -variable - 1;
}

int CDCLSolver::conflict_analysis_and_backtrack(int decision_level){
	vector<int> learnt_clause = literal_list_per_clause[kappa_antecedant];
	...
	clause_count++;
	int backtraacked_decision_level = 0;
	for (int i = 0; i < learnt_clause.size(); i++){
		int literal_index = literal_to_variable_index(learnt_clause[i]);
		int decision_level_here = literal_decision_level[literal_index];
		if (decision_level_here != conflict_decision_level &&
			decision_level_here > backtraacked_decision_level){
			backtraacked_decision_level = decision_level_here;
		}
	}
	for (int i = 0; i < literals.size(); i++){
		if (literal_decision_level[i] > backtraacked_decision_level){
			unassign_literal(i);
		}
	}
	return backtraacked_decision_level;
}

vector<int>& CDCLSolver::resolve(vector<int>& input_clause, int literal){
...
int CDCLSolver::pick_branching_variable() {
...
bool CDCLSolver::all_variables_assigned(){
	return literal_count == assigned_literal_count;
}

void CDCLSolver::show_result(int result_status){
